...
class Evaluation(object):
    ...
    def after_all_episodes(self, episode, rewards_all):
        ...
        #                        sum(r * gamma ** t for t, r in enumerate(rewards_all)), episode)
        ...
        logger.info(f"Episode {episode} Overall score: {sum(sum(rewards_all))}")
        logger.info(f"Episode {episode} Head score: {sum(rewards_all)}")
    # if self.training != True:
    #    df_1 = pd.DataFrame([sum(rewards_all)])
    #   df_1.to_csv(f"/data/home/yael123/multi_head/score_07_08.csv", mode='a', header=False)
