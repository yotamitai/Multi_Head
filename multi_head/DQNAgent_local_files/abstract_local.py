...
class AbstractDQNAgentLocal(AbstractStochasticAgent, ABC):
    def __init__(self, env, config=None):
        super(AbstractDQNAgentLocal, self).__init__(config)
        self.env = env
    ...
    def record(self, state, action, reward_all, next_state, done, info):
        ...
        if isinstance(state, tuple) and isinstance(action, tuple):  # Multi-agent setting
            [self.memory.push(agent_state, agent_action, sum(reward_all), agent_next_state, done, info)
             for agent_state, agent_action, agent_next_state in zip(state, action, next_state)]
        else:  # Single-agent setting
            # yaeli
            # self.memory.push(state, action, reward, next_state, done, info)
            self.memory.push(state, action, sum(reward_all), next_state, done, info)
            self.memory_1.push(state, action, reward_all[0], next_state, done, info)
            self.memory_2.push(state, action, reward_all[1], next_state, done, info)
            self.memory_3.push(state, action, reward_all[2], next_state, done, info)
    ...
    def act(self, state, step_exploration_time=True):
        ...
        self.exploration_policy.update(values[0])
            ...
            #for i in range(1, 4):
           #     df = pd.Series([values[i]])
            #    date = _datetime.date.today()
             #   df.to_csv(f"/data/home/yael123/multi_head/value_file_{date}.csv", mode='a', header=False)
                # print(df)
        return self.exploration_policy.sample()

    def act_test(self, state, step_exploration_time=True):
        """
            Act according to the state-action value model and an exploration policy
        :param state: current state
        :param step_exploration_time: step the exploration schedule
        ...
        # Single-agent setting
        values = self.get_state_action_values(state)
        values = self.get_state_action_values_test(state)
        # yaeli
        # self.exploration_policy.update(values)
        self.exploration_policy.update(values[0])
        #if self.training == False:
    ...
    def get_state_action_values_test(self, state):
        """
        :param state: s, an environment state
        :return: [Q(a1,s), ..., Q(an,s)] the array of its action-values for each actions
        """
